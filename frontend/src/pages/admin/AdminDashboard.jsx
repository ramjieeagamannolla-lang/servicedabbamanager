...
function AdminDashboard() {
    ...
    useEffect(() => {
        const fetchStats = async () => {
            try {
                const [ordersRes, mealsRes, agentsRes] = await Promise.all([
                    API.get('/orders/all'),
                    API.get('/meals'),
                    API.get('/auth/agents')
                ])
        ...
        <div className='min-h-screen bg-orange-50'>
            ...
            <div className='max-w-6xl mx-auto px-4 py-8'>
                ...
                <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
                    ...
                    <Link to='/admin/agents' className='bg-white rounded-xl shadow p-6 hover:shadow-lg transition'>
                        <p className='text-3xl mb-2'>🛵</p>
                        <h3 className='text-xl font-bold text-gray-800'>Manage Agents</h3>
                        <p className='text-gray-500'>Add and view delivery agents</p>
                    </Link>
                </div>
            </div>
        </div>
